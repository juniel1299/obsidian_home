...
- 폴더명을 (.)**가로채고 싶은 화면의 기존 폴더명** 
- ex) book/\[id\] , (.)book/\[id\] 
- 점 하나가 상위로 1칸 올라간다고 생각하면 됨 .
...
## 이미지 최적화
- 일반적으로 이미지는 웹 용량의 58% 차지 하기 때문에 최적화가 필요함 . 
- 이미지 라는 컴포넌트를 이용해서 적용하면 됨 . 
- 현재 보는 페이지의 크기가 아닌 페이지 전체의 이미지를 불러오는 부분 , 이미지 사이즈가 실제 필요한 크기에 비해 더 큰 크기를 불러오는 문제 등등을 해결 할 수 있으
