...
- 동작 흐름 : 사용자 입력 -> Controller 수신 -> Controller 요청 처리 -> Model 상호 작용 (데이터 갱신) -> Model(데이터 상태 변경 및 유지) , Controller에 전송-> Controller는 모델 데이터를 기반으로 뷰 선택 , 응답 생성 -> View 를 통해 표시 
...
### MVC1 , MVC2
- MVC1과 MVC2는 아키텍처와 역할 분리 방식에서 차이가 있다.

- MVC1은 JSP 기반으로 구현 , JSP에서 모델, 뷰, 컨트롤러 모두 담당하고 모델 데이터의 처리, 프레젠테이션 로직을 한 파일에 구현 
- JSP 파일이 모델 데이터를 수신, 비즈니스 로직 처리, HTML 출력 
- 모든 작업이 JSP 파일에서 진행 
- JSP 파일이 요청 처리, 응답 생성함 
- 장점 : 간단한 구조 -> 작은 애플리케이션에 유리 , 빠른 개발 및 배포 가능 
- 단점 : 코드 유지보수 어려움, 확장성 낮음, 재사용성 낮음 


- MVC2는 모델, 뷰, 컨트롤러를 명확히 분리 
- 모델은 비즈니스 로직,데이터 처리 뷰는 프레젠테이션 컨트롤러는 사용자 요청 처리 및 모델 , 뷰 연결
- Servlet , JSP 조합해 구현, 서블릿은 컨트롤러 , 뷰는 JSP 
- 장점 : 코드 유지보수성 향상 , 확장성 높음, 복잡할 수록 유리, 테스트 및 디버깅 유리 
- 단점 : 초기의 설정과 개발 복잡 , 간단한 개발에선 오히려 불편함

MVC1 은 현재 거의 사용 안 함 , MVC2가 많이 사용되며 spring 프레임 워크를 대체적으로 사용
...
### Mybatis, JPA
- Mybatis는 SQL 쿼리를 명시적으로 작성이 가능해 복잡한 쿼리를 효율적으로 작성이 가능하다.
- Mybatis는 SQL 쿼리를 직접 작성한다는 점에서 쿼리 유지보수가 복잡해지고 오버헤드가 증가한다.

- JPA는 객체 지향 모델과 관계형 데이터 베이스 간의 매핑을 표준화 -> 코드 일관성 유지하며 자동으로 매핑을 진행해 Mybatis에 비해 유지 보수가 쉽다는 장점이 있다.
- JPA는 매핑에 대해 오버라이딩이 되어있는 형태이기에 복잡한 쿼리를 작성할 때에 있어 제한이 있거나 매우 어렵다는 단점이 존재한다.


- Mybatis는 설계할 때 SQL 쿼리를 직접 작성하고 XML을 통해 매핑을 해야하며, 그로 인하여 복잡한 쿼리를 효율적으로 작성 가능하다.
- JPA는 ORM 표준 인터페이스로 어노테이션을 통해 매핑이 가능하고, 데이터베이스가 독립적이어서, 쿼리 생성을 자동화하고, SQL 구문을 추상화한다. 
...
### ThymeLeaf
- Java 기반 템플릿 엔진 , HTML 같은 마크업 파일을 템플릿으로 사용하여 동적인 웹 페이지 제작
- EL 을 사용하여 HTML 을 동적으로 렌더링 할 때 유용하고, 템플릿 처리를 효율적으로 관리 할 수 있다.
- 장점 : HTML 기반 템플릿을 이용하므로 개발자와 디자이너 협업에 있어 강점이 있다. 
- 단점 : 복잡한 동적 컨텐츠 처리에 있어서 JavaScript와 통합이 필요함.
...
- 캡슐화 : 데이터를 조작하는 메서드를 하나의 단위로 묶고, 외부 접근을 막아 객체 내부를 보호하는 형태를 의미하며, 일관된 상태를 유지하여 데이터에 대한 무결성 및 일관성 보장이 가능하다.
- 상속 : 코드 재사용성 , 가독성 증가 시키지만 , 심하면 결합도가 너무 높아져 수정이 어려워진다.
- 다형성 : 하나의 인터페이스를 통해 다양한 형태의 객체를 다루는 형식 (부모 - 자식 클래스)으로, 공통 인터페이스를 사용해 다양한 객체를 일관성 있게 다뤄 코드 재사용성이 증대, 객체 간의 결합도 감소로 유연한 설계가 가능 
- 추상화 : 구체적인 구현 세부 사항 숨기고, 인터페이스를 통해 필요 기능만 노출하여 복잡성 줄이고, 공통 인터페이스를 통해 코드 재사용성 향상 증가 
...
##  JavaScript 
### BOM , DOM
- BOM은 브라우저를 제어하고 웹 페이지 외부에서 브라우저와 상호 작용 -> 브라우저 기능 조작
- DOM은 웹 페이지 구조를 조작 

- BOM은 루트가 window , DOM은 루트가 document 
- BOM은 루트가 window이므로 웹 밖에서 제어(탭,히스토리,네비게이션) -> 경고창, URL 변경 
- DOM은 루트가 document 이므로 HTML 문서, 내용 
### Ajax
...
### 동기 , 비동기 
- 동기 프로그래밍은 작업이 순차적으로 실행 -> 각 작업이 완료될 때까지 다음 작업이 대기
- 비동기 프로그래밍은 결과를 기다리지 않고 다음 작업 진행 

- 비동기 프로세싱을 할 경우 작업을 동시에 수행 할 수 있어 응답 시간 단축 및 서버 자원을 효율적으로 활용 가능.. -> 성능 , 확장성 증가 

### 멀티스레딩 , 멀티프로세싱
- 멀티스레딩은 하나의 프로세스 내에서 여러 스레드를 실행 -> 병렬처리
- 멀티프로세싱 여러 프로세스 실행 -> 병렬처리 (프로세스 간의 독립성 유지)

### 단일 책임 원칙(SRP)
- 클래스나 모듈이 하나의 책임만 가진다 -> 변경의 이유가 1개 , 책임 명확해야 함 (유지보수성 증가)

### OAuth2 
- 서드파티 애플리케이션이 자원 소유자의 승인을 통하여 자원 서버에 접근할 수 있는 권한을 부여
- 액세스 토큰을 통해 인증 처리 , 사용자와 서버 간의 보안 상호작용 제공 

### 테스트
- 단위 테스트(Unit Test) 는 메서드 같은 작은 코드를 테스트하여 기능이 올바르게 동작하는지 검증하는 테스트이다.  (JUnit)

- 통합 테스트는 모듈이나 컴포넌트를 통합해 상호작용 테스트와 종단 간의 기능이 정상적으로 동작하는지 검증하는 테스트 Spring Boot Test 같은 도구를 사용 

- 테스트 주도 개발(TDD) : 테스트 케이스를 먼저 작성 후 실패한 테스트를 통과하도록 코드를 작성하는 개발 방법론 
